...
class FoodType(Enum):
  def __init__(self, food_name:str, food_id: int, can_chop: bool, can_cook: bool, buy_cost: int):
    ...
    self.cook_time = can_cook * 20 + can_chop * 2
...
class BotPlayer:
    def __init__(self, map_copy):
        ...
        self.bot_1_work = None
        self.bot_2_work = None
        self.orders = None
        self.state = 0
        self.current_turn = 0
        self.foods = {"EGG": FoodType.EGG, "ONIONS": FoodType.ONIONS, "MEAT": FoodType.MEAT, "NOODLES": FoodType.NOODLES, "SAUCE": FoodType.SAUCE}
    ...
    def process_orders(self, controller: RobotController):
        ...
        for order in orders:
            ...
            for food in order['required']:
                cost += self.foods[food].buy_cost
                est_time += self.foods[food].cook_time
    ...
    def play_turn(self, controller: RobotController):
        ...
        print(self.orders[0]["p_success"], self.orders[0]["is_active"], self.orders[0]["expires_turn"] - (self.orders[0]["est_time"] + self.current_turn))
    
        my_bots = controller.get_team_bot_ids()
        if not my_bots: return

        if self.bot_1_work is None:
            self.bot_1_work = self.orders[0]
        if self.bot_2_work is None:
            self.bot_2_work = self.orders[1]
